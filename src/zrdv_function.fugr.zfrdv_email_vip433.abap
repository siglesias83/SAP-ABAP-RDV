...
*--------------------------------------------------------------------*
  ...
  LOOP AT lt_grp_email INTO ls_grp_email.
    ...
    ELSEIF ls_grp_email-empresa <> '*' AND ls_grp_email-centro_custo ='*'.
      ...
      ENDLOOP.
    ELSEIF ls_grp_email-empresa = '*' AND ls_grp_email-centro_custo <>'*'.
      LOOP AT ztrdv INTO ls_ztrdv WHERE num_centro_custo = ls_grp_email-centro_custo.
        APPEND ls_ztrdv TO lt_ztrdv.
      ENDLOOP.
    ELSE.
      LOOP AT ztrdv INTO ls_ztrdv WHERE empresa = ls_grp_email-empresa
                                    AND num_centro_custo = ls_grp_email-centro_custo.
        APPEND ls_ztrdv TO lt_ztrdv.
      ENDLOOP.
    ENDIF.
    ...
    CONCATENATE '<p>'
                '    Segue erros processamento lançamentos diários dia: '  l_char '.'
                '</p>'
    ...
    LOOP AT lt_ztrdv INTO ls_ztrdv.
      ...
      CONCATENATE '<td align = "LEFT">'
                  '<FONT COLOR = "BLACK">' ls_ztrdv-cartao '</FONT>'
                  '</td>'  INTO gf_objtxt-line.
      APPEND gf_objtxt TO gt_objtxt.
      CLEAR:  gf_objtxt.
    ...
    TRY.
        ...
        CONCATENATE 'RDV - Erro aqruivo lançamentos diários:' ls_ztrdv-arquivo INTO gv_subject SEPARATED BY space.
      ...
      CATCH cx_bcs INTO gr_exception.
        ...
        CONCATENATE 'Email VIP433 -' return-message INTO return-message SEPARATED BY space.
        return-type = 'E'.
    ENDTRY.
  ENDLOOP.
